...
/*   Updated: 2025/02/04 15:02:05 by lilefebv         ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_line_wu(t_point *point_a, t_point *point_b, t_env *env)
{
	...
	if (steep) {
		// Transpose la ligne si elle est plus verticale qu'horizontale
		double temp;
		temp = x0; x0 = y0; y0 = temp;
		temp = x1; x1 = y1; y1 = temp;
	}
	if (x0 > x1) {
		// Assure que la ligne va de gauche à droite
		double temp;
		temp = x0; x0 = x1; x1 = temp;
		temp = y0; y0 = y1; y1 = temp;
		
		int temp2;
		temp2 = c1; c1 = c0; c0 = temp2;
	}
	...
	// Premier pixel avec interpolation
	if (steep) {
		put_pixel_alpha(env, ypxl1, xpxl1, point_a->color, (1.0 - (yend - floor(yend))) * xgap, point_a->z);
		put_pixel_alpha(env, ypxl1 + 1, xpxl1, point_a->color, (yend - floor(yend)) * xgap, point_a->z);
	} else {
		put_pixel_alpha(env, xpxl1, ypxl1, point_a->color, (1.0 - (yend - floor(yend))) * xgap, point_a->z);
		put_pixel_alpha(env, xpxl1, ypxl1 + 1, point_a->color, (yend - floor(yend)) * xgap, point_a->z);
	}
	...
	if (steep) {
		put_pixel_alpha(env, ypxl2, xpxl2, point_b->color, (1.0 - (yend - floor(yend))) * xgap, point_b->z);
		put_pixel_alpha(env, ypxl2 + 1, xpxl2, point_b->color, (yend - floor(yend)) * xgap, point_b->z);
	} else {
		put_pixel_alpha(env, xpxl2, ypxl2, point_b->color, (1.0 - (yend - floor(yend))) * xgap, point_b->z);
		put_pixel_alpha(env, xpxl2, ypxl2 + 1, point_b->color, (yend - floor(yend)) * xgap, point_b->z);
	}
